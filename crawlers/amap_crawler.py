...
class AMapCrawler:
    ...
    def search_poi(self, keyword, city, types=None, extensions="all", boundary=None):
        """
        搜索 POI 数据
        :param keyword: 搜索关键词
        ...
        :param boundary: 地理围栏，格式为 "left,bottom|right,top" 或多边形坐标字符串
        :return: POI 数据列表
        """
        pois = []
        page = 1
        ...
        while has_more:
            ...
            print(f"第 {page} 页: 获取 {current_count} 条，累计 {len(pois)} 条，总计 {count} 条")
    ...
    def search_poi_by_polygon(self, polygon, types=None, extensions="all", city="北京"):
        """
        基于多边形边界搜索 POI 数据（不使用关键词）
        :param polygon: 多边形边界坐标，格式为 "lon1,lat1|lon2,lat2|...|lonN,latN"
        :param types: POI 类型，可选
        :param extensions: 返回结果控制，"base" 或 "all"
        :param city: 城市名称，默认为 "北京"
        :return: POI 数据列表
        """
        # 尝试使用矩形边界格式
        ...
        if len(coords) >= 4:
            ...
            return self.search_poi("", city, types, extensions, boundary)
        else:
            # 构建多边形边界参数
    ...
    def load_boundary_from_file(self, file_path):
        """
        从文件中加载边界坐标
    ...
    def search_poi_by_file(self, file_path, types=None, extensions="all", city="北京"):
        """
        基于文件中的边界搜索 POI 数据（不使用关键词）
        :param file_path: 边界文件路径
        :param types: POI 类型，可选
        :param extensions: 返回结果控制，"base" 或 "all"
        :param city: 城市名称，默认为 "北京"
        ...
        """
        # 加载边界
        polygon = self.load_boundary_from_file(file_path)
        if not polygon:
            print("边界加载失败，无法继续爬取")
            return []
        
        # 使用多边形边界搜索
        return self.search_poi_by_polygon(polygon, types, extensions, city)
